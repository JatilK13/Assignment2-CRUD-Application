...
/*** SERVER READ ***/
// Returns a json object with all the events that match an event title
app.get('/api/events/search/title', async (req, res) => {
    try {
        // Get title from request
        const title = req.query.title;

        // If there is no title, return error status message
        if (!title) {
            return res.status(400).json({ error: "Title of the event is required" });
        }

        // Get all events that match the event title in the database
        const events = await Event.find({title: {$regex: title, $options: 'i'}});

        // If there are no events with the title, return appropriate error status code
        if(events.length === 0) {
            return res.status(404).json({error: "No events found with this title"});
        }

        // Json status code for successful get
        res.status(200).json(events);
    } catch(error) {
        res.status(500).json({error: "Failed to search for events by title"})
    }
});
